...
use gpui::{
    ...
    UTF16Selection, UnderlineStyle, Window, actions, fill, point, prelude::*, px, relative, size,
};
...
/// Fired when the field's text changes (Zed `EditorEvent::Edited`).
#[derive(Clone, Copy, Debug)]
pub enum InputEvent {
    Edited,
}

/// Single-line text field (GPUI `InputHandler`, Zed settings-search inner editor).
pub struct Input {
...
impl EntityInputHandler for Input {
    ...
    fn character_index_for_point(
        ...
        let last_layout = self.last_layout.as_ref()?;
        let utf8_index = last_layout.index_for_x(point.x - line_point.x + self.scroll_x)?;
...
impl Element for InputElement {
    ...
    fn request_layout(
        ...
        style.size.width = relative(1.0_f32).into();
    ...
    fn prepaint(
        ...
        let placeholder_color = Color::Muted.color(cx);
        ...
        let mut scroll_x = px(0.0_f32);
        if cursor_pos + px(2.0_f32) > bounds.size.width {
            scroll_x = cursor_pos + px(2.0_f32) - bounds.size.width;
        }

        let cursor_color = cx.theme().colors().text;
        let selection_color = cx.theme().colors().ghost_element_selected;
            ...
            (
                ...
                Some(fill(
                    Bounds::new(
                        ...
                        size(px(2.0_f32), bounds.size.height),
                    ),
                    cursor_color,
                )),
            )
