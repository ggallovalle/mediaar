...
impl<M: ManagedView> Element for PopoverMenu<M> {
    ...
    fn request_layout(
        ...
        window.with_element_state(
            ...
            |element_state: Option<PopoverMenuElementState<M>>, window| {
                ...
                if self.full_width {
                    style.size = size(relative(1.0_f32).into(), Length::Auto);
                }
    ...
    fn prepaint(
        ...
        let bounds_changed = window.with_element_state(global_id.unwrap(), |element_state, _cx| {
            let mut element_state: PopoverMenuElementState<M> = element_state.unwrap();
            let bounds_changed = element_state.child_bounds != Some(trigger_bounds);
            element_state.child_bounds = Some(trigger_bounds);
            (bounds_changed, element_state)
        });
        if bounds_changed {
            window.refresh();
        }
        ...
        request_layout.child_layout_id.map(|_| {
            window
                .insert_hitbox(trigger_bounds, HitboxBehavior::Normal)
                .id
        })
    }
